...
参考： https://github.com/DXWEIE/AIDM_AFAC_Agent/
...
本文档设计基于对 `AIDM_AFAC_Agent` 项目的拆解与抽象，继承其“任务拆解 + 数据块检索 + 分章节写作 + 图表增强”的核心思路，但重构为更适合工程落地的系统形态。
...
`AIDM_AFAC_Agent` 中东方财富行业研报抓取链路已经验证可用。
